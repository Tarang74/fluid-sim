import { useNavigate } from "react-router-dom";
import "./index.css";

import { useState, type Dispatch, type SetStateAction } from "react";
import { useAuth } from "../../contexts/Auth";
import { loginRequest, initiateGoogleLogin } from "../../api/auth";
import type { User } from "@tarang-and-tina/shared/dist/domain";
import MfaVerify from "../../components/MfaVerify";

export default function LoginForm({
  onSwitch,
  message,
  setMessage,
}: {
  onSwitch: () => void;
  message: ["success" | "error", string];
  setMessage: Dispatch<SetStateAction<["success" | "error", string]>>;
}) {
  const { setAuthenticated, setUser } = useAuth();
  const [usernameOrEmail, setUsernameOrEmail] = useState("");
  const [password, setPassword] = useState("");

  const [mfaVisible, setMfaVisible] = useState(false);
  const [pendingUser, setPendingUser] = useState<User | null>(null);
  const [mfaSession, setMfaSession] = useState("");

  const navigate = useNavigate();

  const handleLogin = async () => {
    try {
      const { user, requiresMfa, session } = await loginRequest(
        usernameOrEmail,
        password,
      );

      if (requiresMfa && session) {
        // MFA required - don't authenticate yet, show MFA popup
        setPendingUser(user);
        setMfaSession(session);
        setMfaVisible(true);
        setMessage(["success", "Please enter your MFA code to complete login"]);
      } else {
        // No MFA required - proceed with normal login
        setAuthenticated(true);
        setUser(user);
        await navigate("/app");
      }
    } catch (err: unknown) {
      setMessage(["error", String(err)]);
    }
  };

  return (
    <>
      <h2 className="login-title">Login</h2>
      <form
        className="login-form"
        method="POST"
        onSubmit={(e) => {
          e.preventDefault();
          void handleLogin();
        }}
      >
        <input
          name="usernameOrEmail"
          type="text"
          value={usernameOrEmail}
          onChange={(e) => {
            setUsernameOrEmail(e.target.value);
          }}
          required
          placeholder="Username or Email"
        />
        <input
          name="password"
          type="password"
          value={password}
          onChange={(e) => {
            setPassword(e.target.value);
          }}
          required
          placeholder="Password"
        />
        <div className="messages">
          <div className={`left ${message[0]}`}>{message[1]}</div>
          <div className="right">
            <span className="link" tabIndex={0}>
              Forgot your password?
            </span>
          </div>
        </div>
        <button type="submit">Take me to the application!</button>

        <button
          type="button"
          className="google-login-btn"
          onClick={() => {
            initiateGoogleLogin();
          }}
        >
          <svg width="20" height="20" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          Continue with Google
        </button>

        <div className="login-switch">
          New here?{" "}
          <span onClick={onSwitch} tabIndex={0}>
            Create an account.
          </span>
        </div>
      </form>

      {/* MFA Verification Popup */}
      {mfaVisible && pendingUser ? (
        <MfaVerify
          pendingUser={pendingUser}
          session={mfaSession}
          setMfaVisible={setMfaVisible}
          onError={(error: string) => {
            setMessage(["error", error]);
          }}
        />
      ) : (
        <></>
      )}
    </>
  );
}
